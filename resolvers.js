...
const resolvers = {
  Query: {
    ...
    buscar: (_, args) => [Profesor.query().findById(1), Curso.query().findById(1)]
  },
  ResultadoBusqueda: {
    __resolveType: (obj) => {
  ...
  Mutation: {
    ...
    profesorDelete: (_, args) => Profesor.query().findById(args.profesorId).then((profesor) => Profesor.query().deleteById(args.profesorId).then((filasBorradas) => {
